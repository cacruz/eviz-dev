# Chapter 8: Model Handler (AbstractRoot)
...
### The Blueprint: AbstractRoot

Just as all the specialized data container classes followed the [DataSource (Base)](05_datasource__base__.md) blueprint, all these specialized Model Handler experts follow a common blueprint called **AbstractRoot**.

**AbstractRoot** is an **Abstract Base Class (ABC)**. It defines the essential actions that *any* Model Handler must be able to perform to be a valid part of the eViz system. It sets the contract, saying "If you are a Model Handler, you *must* have methods for X, Y, and Z." It doesn't provide the *implementation* for these methods; it just declares that they must exist.

For example, a core requirement for any Model Handler is to be able to generate plots. So, `AbstractRoot` defines an abstract `plot()` method. Every specific Model Handler (like the one for `Gridded` data, or `WRF` data) *must* provide its own version of the `plot()` method, containing the specific logic for orchestrating plots for *that* data type.
...
### The Common Ground: The Root Class

While `AbstractRoot` defines the pure blueprint, many data sources share common visualization workflows (like plotting single fields, generating simple plots, or creating comparison plots). To avoid duplicating this common logic in every specific Model Handler (like `Gridded`, `WRF`, `LIS`, etc.), eViz uses an intermediate class called **Root**.

The **Root** class inherits from `AbstractRoot` and provides concrete implementations for these common workflows. It contains the logic for:
...
Most specific Model Handlers in eViz (like `Gridded`, `WRF`, `LIS`, `Geos`) inherit from `Root` to get all this common functionality for free. They then add their *own* specialized logic on top.

### Specialization: Specific Model Handlers (e.g., Gridded, WRF)

Classes like `Gridded`, `WRF`, `LIS`, `Geos`, `Airnow`, etc., are the actual, concrete **Model Handlers**. Each of these classes:

1.  Inherits from `Root` (or sometimes a class that inherits from `Root`, like `NuWrf`).
2.  Implements any abstract methods required by `AbstractRoot` that `Root` didn't cover (though `Root` covers most of the core plotting ones).
...
The main job of an instance of a specific Model Handler (like a `Gridded` object or a `Wrf` object) is to oversee the entire visualization process for the data files assigned to it by the [ConfigManager](02_configmanager_.md).

When [Autoviz (Application Orchestrator)](01_autoviz__application_orchestrator__.md) runs, it first uses the `RootFactory` ([DataSourceFactory](04_datasourcefactory_.md) concept) to create the correct Model Handler instance based on your input (e.g., `-s gridded` creates a `Gridded` instance).

Then, [Autoviz](01_autoviz__application_orchestrator__.md) tells this instance to run its main task, typically by calling its `plot()` method (or its `__call__` method, which in the `Root` class calls `plot()`).
...
for factory in self.factory_sources: # Loop through factories for requested sources
    ...
    # Factory creates the Model Handler instance (e.g., Gridded instance)
...
When `model()` is called, the specific Model Handler instance (inheriting from `Root` which inherits from `AbstractRoot`) takes over and orchestrates the steps:
...
Let's trace the flow focusing on the specific Model Handler (e.g., `Gridded`) coordinating the process after `Autoviz` calls its main method (`model()` -> `Root.plot()`):

```{mermaid}
sequenceDiagram
    participant A as Autoviz
    participant Factory as RootFactory
    participant Handler as Specific Model Handler (e.g., Gridded)
    ...
    Factory-->>Handler: Return Handler instance (e.g., Gridded)
    A->>Handler: handler() (calls handler.plot())
    activate Handler
    Handler->>Handler: (Inherited from Root) Check config type (single/compare)
    Handler->>Handler: (Inherited from Root) Call _single_plots()
    activate Handler
    ...
    loop over fields/levels/times
        ...
        deactivate Plotter
        Handler->>Handler: (Inherited from Root) Call print_map (save/show figure)
    end
    deactivate Handler
...
class AbstractRoot(ABC):
    ...
    # Although implemented in Root now, these were originally abstract
...
This shows that `AbstractRoot` enforces that any class inheriting from it must provide a `logger` and a `plot` method, among others originally defined here.

Next, the `Root` class (in `eviz/models/root.py`), which inherits from `AbstractRoot`:

```python
# eviz/models/root.py (simplified)
from dataclasses import dataclass
import logging
# ... other imports like SinglePlotter, ComparisonPlotter ...
from eviz.lib.config.config_manager import ConfigManager
from eviz.models.base import AbstractRoot # Inherits from AbstractRoot!
# ... other imports ...

@dataclass
class Root(AbstractRoot): # Inherits from AbstractRoot
    """
    Provides common orchestration and plotting logic for Model Handlers.
    Implements the AbstractRoot interface.
    """
    ...
    def __post_init__(self):
        self.logger.info("Start init Root")
    ...
    # This implements the abstract plot() method from AbstractRoot
    def plot(self):
    ...
    # Root also provides concrete implementation for common plotting workflows:
    def _single_plots(self, plotter):
        """Generate single plots based on map_params from config."""
        self.logger.info("Generating single plots (in Root)")
        ...
        for idx, params in self.config_manager.map_params.items():
            ...
            for plot_type in plot_types:
                ...
                # Root calls a helper (_process_plot) which might be implemented
                # differently in Gridded, WRF, etc.
...
This shows `Root` inheriting from `AbstractRoot` and providing the main `plot()` method that dispatches to other helper methods (`_single_plots`, `_comparison_plots`) based on the configuration. It also shows how it retrieves data from the [DataPipeline](03_datapipeline_.md) and prepares to call a plotter. Notice how `_single_plots` calls `_process_plot`, which is a key point for specific handlers to add their logic.

Finally, let's look at the `Gridded` class (in `eviz/models/esm/gridded.py`), which inherits from `Root`:

```python
# eviz/models/esm/gridded.py (simplified)
from dataclasses import dataclass
import logging
# ... other imports like xarray, Figure, DataProcessor ...
from eviz.models.root import Root # Inherits from Root!

@dataclass
class Gridded(Root): # Inherits from Root
    """
    ...
    def __post_init__(self):
        self.logger.info("Start init Gridded")
        super().__post_init__() # Calls Root.__post_init__ and AbstractRoot.__post_init__
        # Gridded might initialize its own DataProcessor or other components
        self.processor = DataProcessor(self.config_manager)


    # Gridded overrides or adds methods to handle its specific data structure
    # This method is called by Root's _single_plots
    def _process_plot(self, data_array: xr.DataArray, field_name: str, file_index: int,
                      plot_type: str, plotter):
        """
        Process a single plot type for a given field (Gridded specific).
        Orchestrates slicing, figure creation, and plotting calls.
        """
        self.logger.info(f"Gridded: Plotting {field_name}, {plot_type} plot")
    ...
    def _get_field_to_plot(self, data_array: xr.DataArray, field_name: str,
        ...
        if 'yz' in plot_type:
            data2d = self._get_yz(data_array, time_lev=time_level) # Gridded specific _get_yz
        elif 'xt' in plot_type:
            data2d = self._get_xt(data_array, time_lev=time_level) # Gridded specific _get_xt
        # ... etc. for tx, xy, polar ...
        elif 'xy' in plot_type or 'polar' in plot_type:
            data2d = self._get_xy(data_array, level=level, time_lev=time_level) # Gridded specific _get_xy
        else: return None
    ...
    # Example of a Gridded specific slicing method
    def _get_xy(self, data_array, level, time_lev):
        """ Extract XY slice (latlon) from a DataArray (Gridded specific logic) """
        if data_array is None: return None
    ...
    # ... Other Gridded specific methods like _get_yz, _get_xt, _get_tx, _select_yrange ...

```

This shows how `Gridded` inherits from `Root`, providing its own `_process_plot` method (which overrides or adds to whatever `Root` might have had, although in this case `Root` delegates this step). This method then calls *further* `Gridded`-specific helper methods like `_get_field_to_plot` and `_get_xy` to perform the actual data slicing and preparation based on its understanding of gridded NetCDF data dimensions and structures. These specific methods then return the prepared data slice and coordinates, which are passed to the [Plotter](07_plotter_.md).

The `Wrf`, `Lis`, `Geos`, `Airnow`, `Omi`, etc., classes follow a similar pattern, inheriting from `Root` (or a class in between) and implementing methods like `_get_field_to_plot` and specific slicing methods (`_get_xy`, `_get_xt`, etc.) with logic tailored to their unique data formats and structures. For instance, the `Wrf` code shows specific handling for staggered grids and pressure levels, while `Lis` has logic for fixing NaN coordinates. `Airnow` and `Omi` handle non-gridded or satellite data structures differently.

### Conclusion

In this chapter, we learned about the **Model Handler**, the crucial layer that provides specialized understanding and orchestration for data coming from different scientific sources. We saw how **AbstractRoot** defines the fundamental contract for any Model Handler, and how the **Root** class provides common plotting workflow implementations that most specific handlers inherit.

Finally, we looked at how concrete Model Handler classes like **Gridded**, **WRF**, and others inherit from `Root` and implement source-specific logic for data slicing, processing, and coordination, ensuring that the entire visualization pipeline correctly handles the unique characteristics of each data type.
